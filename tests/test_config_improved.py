...
class TestConfigStorageUncovered:
    ...
    def test_save_configs_with_complex_objects(self):
        ...
        dashboard_config = DashboardConfig(
            title="テストダッシュボード", layout=LayoutType.FLEX, grid_columns=6
        )

        table_config = TableConfig(
            enable_csv_download=True, sortable=True, page_size=20
        )
...
class TestConfigEdgeCases:
    ...
    def test_dashboard_config_with_enum_values(self):
        """列挙値でのダッシュボード設定テスト"""
        config = DashboardConfig(layout=LayoutType.CUSTOM, title="カスタムレイアウト")
    ...
    def test_filter_config_with_enum_values(self):
        """列挙値でのフィルター設定テスト"""
        config = FilterConfig(
            filter_type=FilterType.MULTISELECT, column="category", allow_multiple=True
        )
    ...
    def test_config_with_none_values(self):
        """None値での設定テスト"""
        config = ComponentConfig(
            component_id=None, title=None, width=None, min_width=None, max_width=None
        )
...
class TestConfigIntegration:
    ...
    def test_complete_config_workflow(self):
        ...
        # 1. デフォルト設定を登録
        dashboard_config = DashboardConfig(
            title="統合テストダッシュボード", layout=LayoutType.GRID, grid_columns=12
        )
        manager.register_default_config("dashboard", dashboard_config)

        # 2. 個別設定を設定
        table_config = TableConfig(
            page_size=15, enable_csv_download=True, sortable=True
        )
