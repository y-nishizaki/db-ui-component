...
class TestDataTransformerFinalUncovered:
    ...
    def test_validate_data_with_special_characters_in_columns(self):
        """特殊文字を含む列名での検証テスト"""
        data = pd.DataFrame(
            {"col-1": [1, 2, 3], "col_2": [4, 5, 6], "col 3": [7, 8, 9]}
        )
    ...
    def test_validate_data_with_categorical_columns(self):
        """カテゴリカル列での検証テスト"""
        data = pd.DataFrame(
            {"category": pd.Categorical(["A", "B", "A", "C"]), "value": [1, 2, 3, 4]}
        )
    ...
    def test_validate_data_with_nan_values(self):
        """NaN値での検証テスト"""
        data = pd.DataFrame({"col1": [1, np.nan, 3], "col2": [4, 5, np.nan]})
        required_columns = ["col1", "col2"]
    ...
    def test_validate_data_with_inf_values(self):
        """無限値での検証テスト"""
        data = pd.DataFrame({"col1": [1, np.inf, 3], "col2": [4, 5, -np.inf]})
        required_columns = ["col1", "col2"]
