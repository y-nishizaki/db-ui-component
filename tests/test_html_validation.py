...
class HTMLValidator(HTMLParser):
    ...
    def __init__(self):
        ...
        self.tags = []
        self.errors = []
        self.self_closing_tags = {'br', 'hr', 'img', 'input', 'meta', 'link', 'area', 'base', 'col', 'embed', 'source', 'track', 'wbr'}
...
class TestHTMLValidation:
    ...
    def test_javascript_injection_prevention(self):
        ...
        malicious_data = pd.DataFrame({
            'name': ['<script>alert("xss")</script>', 'javascript:alert("xss")', 'onclick="alert(\'xss\')"'],
            'value': [1, 2, 3]
        })
    ...
    def test_css_injection_prevention(self):
        ...
        # 悪意のあるCSSコードを含むデータ
        malicious_data = pd.DataFrame({
            'name': ['<style>body{display:none}</style>', 'color:red;position:absolute', 'background:url(javascript:alert("xss"))'],
            'value': [1, 2, 3]
        })
