...
class TestVisualizationConfig:
    ...
    def test_visualization_config_custom_values(self):
        """カスタム値のテスト"""
        config = VisualizationConfig(
            title="Custom Chart", height=600, width=800, background_color="#f0f0f0"
        )
...
class TestSankeyConfig:
    ...
    def test_sankey_config_to_dict(self):
        """設定の辞書変換テスト"""
        config = SankeyConfig(
            title="Sankey Test", node_color="#ff0000", link_color="#00ff00"
        )
        config_dict = config.to_dict()
...
class TestHeatmapConfig:
    ...
    def test_heatmap_config_custom_values(self):
        """カスタム値のテスト"""
        config = HeatmapConfig(
            color_scale="Plasma", show_colorbar=False, colorbar_title="Custom Scale"
        )
...
class TestSankeyDataTransformer:
    ...
    def setup_method(self):
        ...
        self.test_data = pd.DataFrame(
            {"source": ["A", "A", "B"], "target": ["X", "Y", "X"], "value": [10, 5, 8]}
        )
...
class TestNetworkGraphComponent:
    ...
    def setup_method(self):
        ...
        self.component = NetworkGraphComponent(
            source_column="source", target_column="target", title="Test Network"
        )
        self.test_data = pd.DataFrame(
            {
                "source": ["Node1", "Node1", "Node2"],
...
class TestTreemapComponent:
    ...
    def setup_method(self):
        ...
        self.test_data = pd.DataFrame(
            {"labels": ["A", "B", "C"], "parents": ["", "", ""], "values": [20, 30, 15]}
        )
