...
class TableRenderer:
    ...
    def render_table(self, columns: List[str], page_size: int, sortable: bool, height: int) -> str:
        """テーブルをレンダリング"""
        display_data = self.data_manager.get_display_data()
        
        table_html = f'''
        <div class="table-container" style="max-height: {height}px; overflow-y: auto;">
        ...
        # ヘッダーの追加
        for col in columns:
            sort_attr = f' onclick="sortTable(this, {columns.index(col)})"' if sortable else ''
...
class TableComponent:
    ...
    def render(self) -> str:
        ...
        # テーブルの追加
        html_parts.append(self.renderer.render_table(
